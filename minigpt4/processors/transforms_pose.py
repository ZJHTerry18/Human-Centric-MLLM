...
from minigpt4.processors.transforms_post import (affine_transform, fliplr_joints,
    ...
    def __call__(self, results):
        ...
        if np.random.rand() <= self.flip_prob:
            ...
            if isinstance(img, np.ndarray):
                ...
                img = F.hflip(img)
                joints_3d, joints_3d_visible = fliplr_joints(
                    joints_3d, joints_3d_visible, img.size[0],
                    results['ann_info']['flip_pairs'])
                center[0] = img.size[0] - 1 - center[0]
...
class ToUNTensor:
    ...
    def __call__(self, results):
        ...
        if isinstance(image, np.ndarray):
            results['image'] = torch.as_tensor(np.ascontiguousarray(results['image'].transpose(2, 0, 1)),dtype=torch.float32)
        else:
            results['image'] = F.to_tensor(results['image'])
        return results
...
def resize(results, size, max_size=None):
    ...
    def get_size(image_size, size, max_size=None):
        if isinstance(size, (list, tuple, omegaconf.ListConfig)):
            return size[::-1]
        else:
    ...
    if isinstance(image, np.ndarray):
        ...
        rescaled_image = cv2.resize(image, dsize=size, interpolation=cv2.INTER_LINEAR)
        ratios = tuple(float(s) / float(s_orig) for s, s_orig in zip(rescaled_image.shape, image.shape))

        ratio_height, ratio_width, _ = ratios
    else:
        size = get_size(image.size, size, max_size)
        rescaled_image = F.resize(image, size, interpolation=F.InterpolationMode.BICUBIC)
        w, h = image.size
        nw, nh = rescaled_image.size
        ratio_height, ratio_width = float(nh) / float(h), float(nw) / float(w)

    results['image'] = rescaled_image
    results['joints_3d'] = results['joints_3d'] * [ratio_width, ratio_height, 1]
    results['center'] = results['center'] * [ratio_width, ratio_height]
    results['scale'] = results['scale'] * [ratio_width, ratio_height]
...
class Resize(object):
    ...
    def __call__(self, results):
        return resize(results, self.size)
