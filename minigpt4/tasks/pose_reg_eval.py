...
@registry.register_task("pose_reg_eval")
class PoseRegEvalTask(BaseTask):
    ...
    def convert_keypoint(self, image_size, keypoint, gtvis=None):
        joints_pos = np.zeros((len(self.all_keypoint_dict), 2), dtype=np.float32)
        vis_list = np.ones(len(self.all_keypoint_dict)) if gtvis is None else gtvis.cpu().numpy()
        w, h = image_size
        if keypoint is not None:
            joints_pos = keypoint.reshape(len(self.all_keypoint_dict), 2).cpu().numpy()
            joints_pos = joints_pos * np.array([w, h])
    ...
    def valid_step(self, model, samples):
        ...
        gt_keypoints = [samples['all_points'][i] for i in range(len(samples['all_points']))]
        gt_vis = [samples['all_vis'][i] for i in range(len(samples['all_vis']))]

        # run_cfg = slf.cfg.run_cfg
        captions, pred_keypoints = model.generate(
            images,
            texts,
        ...
        target = torch.stack(gt_keypoints)
        pred = torch.cat(pred_keypoints)
        vis = torch.stack(gt_vis).repeat_interleave(repeats=2, dim=1)
        import torch.nn as nn
        l1_loss_fct = nn.SmoothL1Loss(reduction='none', beta=0.002)
        l2_loss_fct = nn.MSELoss(reduction='none')
        l1loss = l1_loss_fct(pred, target) * vis
        l1loss = l1loss.sum() / vis.sum()
        l2loss = l2_loss_fct(pred, target) * vis
        l2loss = l2loss.sum() / vis.sum()
        # print(l1loss, l2loss)
        ...
        for caption, keypoint, vis, img_id, instance_id, ow, oh, bbox in zip(captions, pred_keypoints, gt_vis, img_ids, instance_ids, orig_img_w, orig_img_h, bboxes):
            joints_pos, vis_list = self.convert_keypoint((ow, oh), keypoint, gtvis=vis)
            if len(bbox)>0:
        ...
        for keypoint, vis, img_id, instance_id, ow, oh, bbox in zip(gt_keypoints, gt_vis, img_ids, instance_ids, orig_img_w, orig_img_h, bboxes):
            joints_pos, vis_list = self.convert_keypoint((ow, oh), keypoint, gtvis=vis)
            if len(bbox)>0:
                bbox = [bbox[0]*ow, bbox[1]*oh, bbox[2]*ow, bbox[3]*oh]
